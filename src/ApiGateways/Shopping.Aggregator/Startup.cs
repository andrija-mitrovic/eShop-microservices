    ...
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            services.AddHttpClient<ICatalogService, CatalogService>(x =>
                x.BaseAddress = new Uri(Configuration[Constants.CATALOG_URL]));

            services.AddHttpClient<IBasketService, BasketService>(x =>
                x.BaseAddress = new Uri(Configuration[Constants.BASKET_URL]));

            services.AddHttpClient<IOrderService, OrderService>(x =>
                x.BaseAddress = new Uri(Configuration[Constants.ORDERING_URL]));
